...
ui <- fluidPage(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          fluidRow(
            ...
            radioButtons(
              ...
              choiceValues = list("WFLength", "WFOneSheet",
                                  "WFTwoSheets", "WFCoded"),
              selected = NULL,
              width = "100%"
            ),
        ...
        tabPanel(
          "page_SMob",
          fluidRow(h3("Mobile species data"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$year, {rv$selected$year <- input$year})
  observeEvent(input$month, {rv$selected$month <- input$month})
  observeEvent(input$day, {rv$selected$day <- input$day})
  observeEvent(input$region, {rv$selected$region <- input$region})
  observeEvent(input$subregion, {rv$selected$subregion <- input$subregion})
  observeEvent(input$site, {rv$selected$site <- input$site})
  observeEvent(input$location, {rv$selected$location <- input$location})
  observeEvent(input$habitat, {rv$selected$habitat <- input$habitat})
  observeEvent(input$primsampleu, {rv$selected$primsampleu <- input$primsampleu})
  observeEvent(input$latitude, {rv$selected$latitude <- input$latitude})
  observeEvent(input$longitude, {rv$selected$longitude <- input$longitude})
  ...
  spatialVar <- eventReactive(
    input$verifyVariables,
    {vect <- c(rv$selected$region, rv$selected$subregion,
  ...
  longFishTable <- eventReactive(
    input$longFishSave, {
      if(calcBiomass() == "Yes"){
        if(codedLength() == "No"){
          options(warn = -1)
          fishTable <- f_cBiomass(Intro$InputDT(), longFishLength(),
                                  longFishNames()) |>
  ...
  observeEvent(input$page_SMob_skip, {skipMobFlag(TRUE)})
  observeEvent(input$page_SLPI_SMob, {skipMobFlag(FALSE)})
  observeEvent(input$page_WMob_SMob, {skipMobFlag(FALSE)})
  observeEvent(input$page_LMob_SMob, {skipMobFlag(FALSE)})
  observeEvent(input$page_SCol_xFish, {skipMobFlag(FALSE)})
  observeEvent(input$longMobSave, {skipMobFlag(FALSE)})
  observeEvent(input$wideMobSave, {skipMobFlag(FALSE)})
  ...
  observeEvent(input$page_SLPI_skip, {skipLPIFlag(TRUE)})
  observeEvent(input$page_SDEMO_SLPI, {skipLPIFlag(FALSE)})
  observeEvent(input$page_WMob_SLPI, {skipLPIFlag(FALSE)})
  observeEvent(input$page_LMob_SLPI, {skipLPIFlag(FALSE)})
  observeEvent(input$page_SMob_skip, {skipLPIFlag(FALSE)})
  observeEvent(input$page_WLPI_SLPI, {skipLPIFlag(FALSE)})
  observeEvent(input$page_LLPI_SLPI, {skipLPIFlag(FALSE)})
  observeEvent(input$longLPISave, {skipLPIFlag(FALSE)})
  observeEvent(input$wideLPISave, {skipLPIFlag(FALSE)})
  ...
  observeEvent(input$page_SDEMO_skip, {skipDEMOFlag(TRUE)})
  observeEvent(input$page_DlBCG_Ini, {skipDEMOFlag(FALSE)})
  observeEvent(input$page_SLPI_SDEMO, {skipDEMOFlag(FALSE)})
  observeEvent(input$page_WLPI_SDEMO, {skipDEMOFlag(FALSE)})
  observeEvent(input$page_LLPI_SDEMO, {skipDEMOFlag(FALSE)})
  observeEvent(input$page_WDEMO_SDEMO, {skipDEMOFlag(FALSE)})
  observeEvent(input$page_LDEMO_SDEMO, {skipDEMOFlag(FALSE)})
  observeEvent(input$longDEMOSave, {skipDEMOFlag(FALSE)})
  observeEvent(input$wideDEMOSave, {skipDEMOFlag(FALSE)})
  ...
  wideDEMOTable <- eventReactive(
    input$wideDEMOSave, {
      f_W2LDEMO(DEMO$InputDT(), vectorSel(),
                timeVar(), spatialVar(),
  ...
  ModelShort <- reactive({
    if(Model() == "BCG Fish") {"FishData_"}
    else if(Model() == "BSAT (LPI)") {"BSATData_"}
    else {"BCGData_"}
  })
